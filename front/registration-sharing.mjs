...
function decorateRegistrationSection(section) {
  ...
  if (!sessionUser) {
    if (title && title.textContent !== 'Mon inscription') title.textContent = 'Mon inscription';
    return;
  }
  if (title && title.textContent !== 'Inscriptions') title.textContent = 'Inscriptions';
  ...
  if (oldActions && !oldActions.hidden) oldActions.hidden = true;
  const addButton = toolbar.querySelector('[data-registration-add]');
  const shouldDisable = departure.startsAt <= Date.now();
  if (addButton && addButton.disabled !== shouldDisable) addButton.disabled = shouldDisable;
...
async function start() {
  ...
  decorate();
  if (app) {
    appObserver = new MutationObserver(scheduleDecorate);
    observeApp();
  }
}
