...
(async()=>{try{const result=await telemetry('/client-report/error?view=admin');render(Array.isArray(result.errors)?result.errors:[]);}catch(error){app.innerHTML=`<section class="members-panel members-error"><h1>Accès impossible</h1><p>${esc(error.message||String(error))}</p><a class="secondary-button" href="/">Retour à l’accueil</a></section>`;}})();
