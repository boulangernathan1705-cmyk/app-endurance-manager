...
if (shell && app && navigation) {
  const toolbar = document.createElement('div');toolbar.className = 'desktop-home-toolbar';toolbar.setAttribute('aria-label','Actions principales');
  const center = document.createElement('div');center.className = 'desktop-nav-center';center.setAttribute('aria-label','Changer de simulateur');
  const right = document.createElement('div');right.className = 'desktop-nav-right';const rightControls = document.createElement('div');rightControls.className = 'desktop-nav-right-controls';right.append(rightControls);
  shell.insertBefore(toolbar, navigation);shell.insertBefore(center, navigation);shell.insertBefore(right, navigation);
  const accountBar = shell.querySelector(':scope > .account-bar');const cloneControl = node => node ? node.cloneNode(true) : null;
  function restoreMobileAccount() { if (accountBar && accountBar.parentElement !== shell) shell.append(accountBar); }
  function sync() {
    const title = app.querySelector(':scope > .page-title'),create = app.querySelector(':scope > .home-create-event'),filters = app.querySelector(':scope > .event-filter');
