...
const EN=new Map(Object.entries({
  ...
  'Rechercher un pilote…':'Search a driver…','Rechercher un membre':'Search a member','Enregistrement…':'Saving…','✓ Enregistré':'✓ Saved','Aucun membre ne correspond à cette recherche.':'No member matches this search.','Commun à LMU et iRacing. Un pilote apparaît ici après sa première connexion Discord ; son rôle est enregistré dès que tu le changes. « Pilote SAFE » donne accès aux courses solo SAFE.':'Shared by LMU and iRacing. A driver appears here after their first Discord sign-in; their role is saved as soon as you change it. “SAFE driver” gives access to SAFE solo races.',
  // Solo races
  'Catégories de la course':'Race categories','Catégorie et voiture':'Category and car','voiture : peu importe':'car: any','Choisis une catégorie pour chaque manche.':'Choose a category for each round.',
  'Courses solo':'Solo races','COURSES SOLO':'SOLO RACES','Ajouter une course solo':'Add a solo race','ENDURANCE':'ENDURANCE',
  'Pilote SAFE':'SAFE driver','Accès aux courses solo SAFE':'Access to SAFE solo races','Je participe':'I’m in','JE PARTICIPE':'I’M IN',
  'Connecte-toi avec Discord pour participer.':'Sign in with Discord to take part.','Course réservée aux pilotes SAFE.':'Race reserved to SAFE drivers.',
  'Participants':'Participants','Liste d’attente':'Waiting list','Le premier en attente prend automatiquement la place d’un pilote qui se désinscrit.':'The first driver waiting automatically takes the place of a driver who withdraws.',
  'Aucun inscrit pour l’instant.':'No entry yet.','Peu importe':'Any','N’importe quelle catégorie':'Any category','Catégorie « Peu importe » : la voiture est libre aussi.':'Category “Any”: the car is free too.',
  'Les inscriptions sont fermées.':'Entries are closed.','Réservée aux pilotes SAFE':'Reserved to SAFE drivers','Ouverte à tous les pilotes connectés':'Open to every signed-in driver',
  'Réservée aux pilotes SAFE.':'Reserved to SAFE drivers.','Ouverte à tous les pilotes connectés.':'Open to every signed-in driver.',
  'Endurance':'Endurance','Course solo':'Solo race','Équipages, relais et heures de présence.':'Crews, stints and hours present.','Une inscription par pilote, places limitées, une ou deux manches.':'One entry per driver, limited places, one or two rounds.',
  'Format :':'Format:','Durée (minutes)':'Duration (minutes)','Circuit aléatoire (annoncé au dernier moment)':'Random track (announced at the last minute)','Circuit aléatoire':'Random track','Circuit à confirmer':'Track to be confirmed',
  'Deux manches':'Two rounds','Deux courses courtes le même soir, avec une seule inscription.':'Two short races the same evening, with a single entry.','Nombre de places':'Number of places',
  'Au-delà, les pilotes peuvent s’inscrire en liste d’attente. Proposé d’après la taille du serveur de jeu (62 au Mans, 38 ailleurs sur LMU).':'Beyond that, drivers can join the waiting list. Suggested from the game server size (62 at Le Mans, 38 elsewhere on LMU).',
  'Accès':'Access','Manches':'Rounds','Manche':'Round','Places':'Places','inscrits':'drivers','inscrit':'driver','complet':'full',
  'Connecte-toi avec Discord pour t’inscrire à une course solo.':'Sign in with Discord to enter a solo race.','Seuls les organisateurs peuvent inscrire un autre pilote à une course solo.':'Only organizers can register another driver for a solo race.',
  'Cette course est réservée aux pilotes SAFE. Demande à un administrateur de t’ajouter.':'This race is reserved to SAFE drivers. Ask an administrator to add you.','Ce pilote est déjà inscrit à cette course. Modifie son inscription.':'This driver is already entered in this race. Edit their entry.',
  'Les courses solo n’ont pas d’équipage.':'Solo races have no crews.','Une course solo a un seul départ.':'A solo race has a single start.','Choisis l’accès OPEN ou SAFE.':'Choose OPEN or SAFE access.',
  'Une course solo a une ou deux manches.':'A solo race has one or two rounds.','Les deux manches doivent être sur le même simulateur.':'Both rounds must be on the same simulator.','Le nombre de places doit être compris entre 2 et 120.':'The number of places must be between 2 and 120.',
  'Une course solo ne peut pas dépasser 24 heures.':'A solo race cannot last more than 24 hours.','Choisis une catégorie de cette course, ou « Peu importe ».':'Choose a category of this race, or “Any”.','Choisis le circuit de chaque manche.':'Choose the track of each round.','Gestion des membres':'Member management','Administrateur principal':'Main administrator','Organisateur':'Organizer',
...
const PATTERNS=[
  [/^Liste d’attente · (\d+)(?:er|e)$/u,([,n])=>`Waiting list · #${n}`],
  [/^✓ Inscrit · (.+)$/u,([,category])=>`✓ Registered · ${category==='Peu importe'?'Any':category}`],
  [/^Manche (\d)$/u,([,n])=>`Round ${n}`],
  [/^Manche (\d) · (.+)$/u,([,n,rest])=>`Round ${n} · ${rest}`],
  [/^Catégories manche (\d)$/u,([,n])=>`Round ${n} categories`],
  [/^Choisis au moins une catégorie pour la manche (\d)\.$/u,([,n])=>`Choose at least one category for round ${n}.`],
  [/^Choisis une catégorie pour la manche (\d)(?:, ou « Peu importe »)?\.$/u,([,n])=>`Choose a category for round ${n}.`],
  [/^Choisis au moins une voiture pour la manche (\d), ou « Peu importe la voiture »\.$/u,([,n])=>`Choose at least one car for round ${n}, or “Any car”.`],
  [/^Choisis uniquement des voitures proposées pour la manche (\d)\.$/u,([,n])=>`Only choose cars offered for round ${n}.`],
  [/^(\d+) en attente$/u,([,n])=>`${n} waiting`],
  [/^(\d+) \(puis liste d’attente\)$/u,([,n])=>`${n} (then waiting list)`],
  [/^La course est complète : (tu seras|ce pilote sera) en liste d’attente \((\d+)(?:er|e)\)\. En cas de désistement, la place revient automatiquement au premier en attente\.$/u,([,who,n])=>`The race is full: ${who==='tu seras'?'you will be':'this driver will be'} on the waiting list (#${n}). If someone withdraws, the first driver waiting gets the place automatically.`],
  [/^Choisis le circuit de la manche (\d)\.$/u,([,n])=>`Choose the track of round ${n}.`],
  [/^La durée de la manche (\d) doit être comprise entre 5 et 600 minutes\.$/u,([,n])=>`Round ${n} must last between 5 and 600 minutes.`],
