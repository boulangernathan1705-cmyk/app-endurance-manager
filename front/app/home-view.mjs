...
const shortDateFormatter=new Intl.DateTimeFormat('fr-FR',{timeZone:'Europe/Paris',day:'numeric',month:'short'});
function dateParts(timestamp){const values={};for(const part of dateFormatter.formatToParts(new Date(timestamp)))if(part.type!=='literal')values[part.type]=part.value;return values;}
function dateRange(departures=[]){const dated=departures.filter(d=>Number.isFinite(Number(d.startsAt))).sort((a,b)=>Number(a.startsAt)-Number(b.startsAt));if(!dated.length)return'Dates à confirmer';const unique=[],seen=new Set();for(const d of dated){const item=dateParts(Number(d.startsAt)),key=`${item.year}-${item.month}-${item.day}`;if(!seen.has(key)){seen.add(key);unique.push(item);}}const first=unique[0],last=unique.at(-1);if(unique.length===1)return`${first.day} ${first.month} ${first.year}`;if(first.month===last.month&&first.year===last.year)return`${first.day}–${last.day} ${first.month} ${first.year}`;if(first.year===last.year)return`${first.day} ${first.month} – ${last.day} ${last.month} ${first.year}`;return`${first.day} ${first.month} ${first.year} – ${last.day} ${last.month} ${last.year}`;}
...
function scheduleLabel(event){const dates=dateRange(event.departures||[]),times=[...new Set((event.departures||[]).map(d=>displayTime(d.time)).filter(Boolean))];return times.length?`${dates} · ${times.slice(0,4).join(' • ')}${times.length>4?' • …':''}`:dates;}
...
function crewRows(event){
  ...
  for(const departure of [...(event.departures||[])].filter(d=>Number.isFinite(Number(d.startsAt))).sort((a,b)=>Number(a.startsAt)-Number(b.startsAt))){
    for(const crew of departure.crews||[]){
      ...
      rows.push({startsAt:Number(departure.startsAt),time:departure.time,category:crew.category||registrations[0]?.category||'',name:crew.name||'Équipage',car:crew.car||'',count:registrations.length});
    }
  }
  const categoryOrder=category=>{const index=(event.categories||[]).indexOf(category);return index<0?999:index;};
  return rows.sort((a,b)=>a.startsAt-b.startsAt||categoryOrder(a.category)-categoryOrder(b.category)||String(a.name).localeCompare(String(b.name),'fr',{sensitivity:'base',numeric:true}));
}
function crewSummary(event){
  const rows=crewRows(event);if(!rows.length)return'';
  return `<span class="event-crew-summary"><span class="event-crew-summary-title">Équipages engagés</span><span class="event-crew-summary-list">${rows.map(row=>`<span class="event-crew-summary-row"><span class="event-crew-when">${esc(shortDateFormatter.format(new Date(row.startsAt)))} · ${esc(displayTime(row.time))}</span><span class="event-crew-category">${esc(row.category)}</span><span class="event-crew-name">${esc(row.name)}</span>${row.car?`<span class="event-crew-car">${esc(row.car)}</span>`:''}<span class="event-crew-count">${row.count} pilote${row.count>1?'s':''}</span></span>`).join('')}</span></span>`;
}
...
function eventCard({event,next,running,archived,end}){
  ...
  const status=archived?`Terminé le ${esc(dateLabel({startsAt:end}))}`:displayRunning?'<span class="event-countdown event-countdown-primary running">COURSE EN COURS</span>':displayNext?`Prochain départ avec pilotes : ${esc(dateLabel(displayNext))} à ${esc(displayNext.time)} · <span data-countdown="${displayNext.startsAt}">${countdown(displayNext.startsAt)}</span>`:next?'Aucun départ à venir avec pilote inscrit':'Dates à confirmer';
  return `<button class="event-card event-card-harmonized event-type-${event.eventType||'private'} ${archived?'archived':''}" data-action="open" data-id="${event.id}"><span class="event-card-body"><span class="event-card-title-row"><span class="event-name">${esc(event.name)}</span><span class="event-info">${eventTypeBadge(event.eventType)} <span class="event-home-schedule">${esc(scheduleLabel(event))}</span></span></span>${circuitVisual(event.circuit,true)}<span class="event-category-badges">${event.categories.map(category=>eventBadge(category,eventCategoryCount(event,category))).join('')}</span>${crewSummary(event)}<span class="event-card-status"><span class="event-countdown ${statusClass}" ${displayNext&&!archived?`data-status-time="${displayNext.startsAt}"`:''}>${status}</span></span></span></button>`;
}
