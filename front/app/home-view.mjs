...
import {joinedOrganizations,registrationAudienceIds} from './organization-context.mjs?v=4-paddock-network';
...
function renderPaddockPulse(){
  ...
  return `<section class="paddock-pulse"><div class="paddock-pulse-head"><div><span>PADDOCK</span><strong>Ce qui mérite ton attention</strong></div><button type="button" class="secondary-button" data-network-open>Mon réseau</button></div>${networkChips()}<div class="paddock-pulse-grid"><div class="paddock-next"><span class="paddock-panel-label">MON PROCHAIN ENGAGEMENT</span>${nextMarkup}</div><div class="paddock-signals"><span class="paddock-panel-label">SIGNAUX DU RÉSEAU</span>${signalsMarkup}</div></div></section>`;
}
