...
function crewRows(event,category){
  ...
  for(const departure of [...(event.departures||[])].filter(d=>Number.isFinite(Number(d.startsAt))).sort((a,b)=>Number(a.startsAt)-Number(b.startsAt))){
    for(const crew of departure.crews||[]){
      ...
      rows.push({startsAt:Number(departure.startsAt),time:departure.time,name:crew.name||'Équipage',car:crew.car||''});
    }
  }
  return rows.sort((a,b)=>a.startsAt-b.startsAt||String(a.name).localeCompare(String(b.name),'fr',{sensitivity:'base',numeric:true}));
}
function categoryBadge(event,category){
  const count=eventCategoryCount(event,category),rows=crewRows(event,category);
  return `<span class="event-category-badge ${categories[category]?.css||''}">${logo(category)}<span class="event-category-copy"><strong>${esc(category)}</strong><small>${count} inscrit${count>1?'s':''}</small>${rows.length?`<span class="event-category-crews">${rows.map(row=>`<span class="event-category-crew"><b>${esc(shortDateFormatter.format(new Date(row.startsAt)))} · ${esc(displayTime(row.time))}</b><span>${esc(row.name)}${row.car?` · ${esc(row.car)}`:''}</span></span>`).join('')}</span>`:''}</span></span>`;
}
...
function eventCard({event,next,running,archived,end}){
  ...
  return `<button class="event-card event-card-harmonized event-type-${event.eventType||'private'} ${archived?'archived':''}" data-action="open" data-id="${event.id}"><span class="event-card-body"><span class="event-card-title-row"><span class="event-name">${esc(event.name)}</span><span class="event-info">${eventTypeBadge(event.eventType)} <span class="event-home-schedule">${esc(scheduleLabel(event))}</span></span></span>${circuitVisual(event.circuit,true)}<span class="event-category-badges">${event.categories.map(category=>categoryBadge(event,category)).join('')}</span><span class="event-card-status"><span class="event-countdown ${statusClass}" ${displayNext&&!archived?`data-status-time="${displayNext.startsAt}"`:''}>${status}</span></span></span></button>`;
}
