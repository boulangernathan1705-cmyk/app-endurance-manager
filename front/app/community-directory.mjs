...
function teamPanel(){
  ...
  if(!team)return `<section class="community-directory-section team-secondary"><div class="community-section-heading"><div><small>TEAM PRIVÉE · FACULTATIF</small><h2>Ton groupe fixe</h2><p>Une Team reste un espace privé pour tes coéquipiers habituels. Elle n’est pas publiée dans l’annuaire des communautés.</p></div></div><form data-community-form="team" class="community-team-create"><input name="name" maxlength="60" required placeholder="Nom de la Team"><button class="secondary-button" type="submit">Créer ma Team</button></form></section>`;
  const ids=new Set((team.members||[]).map(member=>member.id));
  const candidates=(state.participants||[]).filter(p=>!ids.has(p.id));
  return `<section class="community-directory-section team-secondary"><div class="community-section-heading"><div><small>TEAM PRIVÉE</small><h2>${esc(team.name)}</h2><p>${Number(team.memberCount)||0} membre${Number(team.memberCount)===1?'':'s'} · espace non publié dans l’annuaire.</p></div></div>${memberList(team)}${canManage(team)?`<form data-community-form="team-member" data-id="${team.id}" class="community-team-create"><select name="userId" required><option value="">Ajouter un pilote…</option>${candidates.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('')}</select><button class="secondary-button" type="submit" ${candidates.length?'':'disabled'}>Ajouter</button></form>`:''}</section>`;
}
