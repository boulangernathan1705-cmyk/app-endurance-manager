...
function membersSection(community){
  ...
  return `<section class="community-detail-block community-members-block"><div class="community-block-heading"><div class="community-block-title"><small>MEMBRES</small><h2>${count} membre${count===1?'':'s'}</h2></div>${community.role?`<button type="button" class="community-icon-action" data-community-action="refresh-members" data-id="${community.id}" aria-label="Actualiser les membres">Actualiser</button>`:''}</div>${community.role?memberList(community):'<p class="community-muted">Les membres deviennent visibles après avoir rejoint la communauté.</p>'}${community.discord?.linked?'<p class="community-members-help">Les membres Discord apparaissent ici après leur première connexion au site.</p>':''}</section>`;
}
...
function joinAction(community){
  if(community.role)return `<div class="community-profile-actions-row">${community.id!==state.organizations?.preferredCommunityId?`<button class="primary-button" data-community-action="set-default" data-id="${community.id}">Définir par défaut</button>`:'<span class="community-status-pill">Espace par défaut</span>'}${community.role==='owner'?'<span class="community-status-pill">Responsable</span>':`<button class="secondary-button" data-community-action="leave" data-id="${community.id}">Quitter</button>`}</div>`;
  ...
  if(community.joinMode==='invite')return'<span class="community-status-pill">Nouvelles adhésions fermées</span>';
