...
import {renderPilots} from './crews.mjs?v=4-crew-actions';
...
function departureActions(departure,own){
  const crewAction=canCreateCrewOnDeparture(departure)?`<button type="button" class="primary-button crew-builder-open departure-create-crew" data-crew-builder-open data-departure="${departure.id}">${canManage()?'Créer un équipage':'Créer mon équipage'}</button>`:'';
  const primary=`${button('my-registration',own?'Modifier mon inscription':'S’inscrire',`data-departure="${departure.id}"`,'primary-button departure-self-registration')}${crewAction}`;
  const other=state.user?button('new-registration','Inscrire un autre pilote',`data-departure="${departure.id}" data-mode="pilot"`,'secondary-button departure-other-registration'):'';
  return `<div class="departure-action-panel"><div class="departure-primary-actions">${primary}</div>${other?`<div class="departure-secondary-actions">${other}</div>`:''}</div>`;
}
