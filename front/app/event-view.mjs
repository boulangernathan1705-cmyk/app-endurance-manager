...
import {renderHome,showRecoveryLink} from './home-view.mjs?v=6-teams-core';
import {scopeEvent,defaultCrewOrganizationId,joinedOrganizations,organizationById} from './organization-context.mjs?v=3-teams-core';

function managesAnyGroup(){
  return joinedOrganizations(state.organizations).some(organization=>['owner','manager'].includes(organization.role));
}
...
function groupContextMarkup(){
  const organization=organizationById(state.organizations,state.activeOrganizationId);
  if(!organization)return'';
  const kind=organization.type==='team'?'Team privée':'Communauté';
  return `<section class="event-group-context ${organization.type}" aria-label="Contexte ${esc(organization.name)}"><div><span>${kind}</span><strong>${esc(organization.name)}</strong><small>Cette vue affiche uniquement les inscriptions et équipages partagés avec ${esc(organization.name)}.</small></div><button type="button" class="secondary-button" data-teams-action="back-detail" data-organization-id="${organization.id}">Retour à ${esc(organization.name)}</button></section>`;
}
