...
export function renderDeparturePanel(event,departure,index,open=false,{isNext=false,isPast=false}={}){
...
export function renderEvent(message=''){
  ...
  const upcoming=visible.map(({departure,index})=>renderDeparturePanel(event,departure,index,departure.id===state.selectedDepartureId||state.registrationOpen.has(departure.id),{isNext:departure.id===next?.id})).join('');
