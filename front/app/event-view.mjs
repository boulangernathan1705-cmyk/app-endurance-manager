...
export function renderDeparturePanel(event,departure,index,open=false,{isPast=false}={}){
  ...
  const createCrewAction=canCreateCrew?`<button type="button" class="secondary-button ux-summary-create-crew" data-crew-builder-open data-departure="${departure.id}">${esc(canManage()?'Créer un équipage':'Créer mon équipage')}</button>`:'';
