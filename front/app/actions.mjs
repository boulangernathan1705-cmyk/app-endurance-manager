...
async function finishPendingCrewJoin(departureId,registrationId){
  const pending=state.pendingCrewJoin;
  if(!pending||pending.departureId!==departureId||!registrationId)return false;
  try{
    await load();
    const event=state.events.find(item=>item.id===pending.eventId);
    const departure=event?.departures.find(item=>item.id===pending.departureId);
    const crew=departure?.crews.find(item=>item.id===pending.crewId);
    if(!crew)throw Error('Cet équipage n’existe plus.');
    await api(`/api/crews/${crew.id}/members`,'POST',{registrationId,version:crew.version,selfJoin:true});
    state.crewManagementOpen.add(crew.id);
    state.pendingCrewJoin=null;
    await refreshAfterSave(`Inscription enregistrée · équipage « ${crew.name} » rejoint.`);
    return true;
  }catch(error){
    state.pendingCrewJoin=null;
    await refresh();
    throw Error(`Ton inscription est bien enregistrée, mais tu n’as pas pu rejoindre l’équipage : ${error.message}`);
  }
}
...
async function perform(action,target){
  ...
  switch(action){
    ...
    case 'join-crew': {
      ...
      if(!reg){beginCrewJoin(event,departure,crew);break;}
...
async function start(){
  try{const token=new URLSearchParams(location.hash.slice(1)).get('access');if(token){history.replaceState(null,'',location.pathname+location.search);await api('/api/guest/recover','POST',{token});state.flash='Tes inscriptions invitées sont accessibles sur cet appareil.';}const authError=new URLSearchParams(location.search).get('auth');if(authError){history.replaceState(null,'',location.pathname);state.flash='La connexion Discord n’a pas abouti. Tu peux réessayer.';}const returnEventId=new URLSearchParams(location.hash.slice(1)).get('event');if(returnEventId)history.replaceState(null,'',location.pathname+location.search);await load();renderNav();if(returnEventId&&state.events.some(item=>item.id===returnEventId)){state.currentEventId=returnEventId;state.selectedDepartureId=null;renderEvent(state.flash);}else renderHome(state.flash);}catch(error){app.innerHTML='<h1 class="page-title">ENDURANCE MANAGER</h1>';showError(error);}
}
start();
