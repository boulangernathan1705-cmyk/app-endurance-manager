...
function crewMarkup(crew, departure) {
  ...
  return `<li class="hub-crew ${crew.locked ? 'is-complete' : 'is-open'}">
    <span class="hub-crew-status" aria-label="${crew.locked ? 'Équipage complet' : 'Équipage ouvert'}">${crew.locked ? '🔒' : '🔓'}</span>
...
function enduranceMarkup(item, game) {
  ...
  return `<section class="hub-next-race" aria-label="Prochaine endurance ${esc(catalog.name)}">
    ...
    ${crews.length ? `<ul class="hub-crews">${crews.map(crew => crewMarkup(crew,departure)).join('')}</ul>` : '<p class="hub-no-crews">Aucun équipage formé pour ce départ.</p>'}
  </section>`;
}
