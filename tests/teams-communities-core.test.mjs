...
test('les assets de la nouvelle interface sont versionnés et les fixtures dev restent isolées',()=>{
  ...
  assert.match(html,/community-context\.css\?v=5-site-tool/);
  assert.match(html,/community-directory\.css\?v=8-site-tool/);
  assert.match(html,/app\.js\?v=102-tondeuz-tool/);
  assert.match(app,/paddock-network\.mjs\?v=13-tondeuz-tool/);
});
