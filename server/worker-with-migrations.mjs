...
import {ensureDevOrganizations} from './dev-seed-organizations.mjs';
...
async function ensureCrewOwnershipSchema(env) {
  ...
  crewOwnershipReady = (async () => {
    ...
    await env.DB.prepare(`UPDATE crews
      SET owner_user_id = (
        SELECT p.user_id FROM crew_members cm
        JOIN registrations r ON r.id = cm.registration_id
        JOIN participants p ON p.id = r.participant_id
        WHERE cm.crew_id = crews.id AND p.user_id IS NOT NULL
        ORDER BY r.created_at,r.id LIMIT 1
      ) WHERE owner_user_id IS NULL`).run();
    try { await env.DB.prepare("INSERT OR IGNORE INTO d1_migrations(name) VALUES('0016_crew_ownership.sql')").run(); } catch {}
  })().catch(error => { crewOwnershipReady = null; throw error; });
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname.startsWith('/api/')) {
      ...
      await ensureDevOrganizations(request,env);
    }
