...
function renderPilots(event,departure) {
  return `<div class="pilot-section">
    ${event.categories.map(category=>{const regs=departure.availability.filter(r=>r.category===category&&r.status!=='unavailable'),allCrews=departure.crews||[],crews=allCrews.filter(crew=>crew.category===category),assigned=new Set(crews.flatMap(crew=>crew.registrationIds)),unassigned=regs.filter(reg=>!assigned.has(reg.id));if(!regs.length)return '';return `<div class="category-group"><div class="category-group-header ${categories[category]?.css||''}">${logo(category)}<span>${esc(category)} · ${regs.length} pilote${regs.length>1?'s':''}</span></div>${crews.map(crew=>{const crewRegs=crew.registrationIds.map(id=>regs.find(reg=>reg.id===id)).filter(Boolean);return crewRegs.length?`<details class="crew-pilot-group ${crewColorClass(crew.id,allCrews.indexOf(crew))}"><summary class="crew-pilot-summary"><span class="crew-pilot-toggle" aria-hidden="true"></span><strong class="crew-summary-name">${esc(crew.name)}</strong><span class="crew-summary-category">${logo(category)}</span><span class="crew-summary-pilots" title="${esc(crewRegs.map(reg=>reg.name).join(' · '))}">${crewRegs.map(reg=>esc(reg.name)).join(' · ')}</span><span class="crew-summary-car">${esc(crew.car||'Voiture à choisir')}</span></summary><div class="crew-pilot-details">${crewRegs.map(reg=>renderRegistration(reg,departure,event.durationHours||6,false)).join('')}</div></details>`:'';}).join('')}${unassigned.map(reg=>renderRegistration(reg,departure,event.durationHours||6)).join('')}</div>`;}).join('')}
